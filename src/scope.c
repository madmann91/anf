...
void scope_compute(mod_t* mod, const fn_t* fn, node_set_t* scope) {
    node_vec_t worklist = node_vec_create(64);
    node_set_insert(scope, &fn->node);
...
void scope_compute_fvs(const fn_t* fn, node_set_t* scope, node_set_t* fvs) {
    node_set_t done = node_set_create(64);
    node_vec_t worklist = node_vec_create(64);
    for (size_t i = 0; i < fn->node.nops; ++i)
        node_vec_push(&worklist, fn->node.ops[i]);
    while (worklist.nelems > 0) {
        const node_t* node = worklist.elems[--worklist.nelems];
